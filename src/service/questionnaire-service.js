...
export default {
	...
	insertQuestionnaireResult: async (quesResult, uuid) => {
		...
		if (!status[0].isEvaluate) {
			...
			return {
				status: await questionnaireDao.insertQuestionnaireResult(quesResult.join(','), uuid)
			};
		} else {
			return {
				status: '502'
	...
	getRank: async (uuid) => {
		...
		quesStandard = quesStandard.map((item) => {
			item.rank = 0;
			item.select_is_question_num = item.select_is_question_num.toString().split(',');
			item.select_no_question_num = item.select_no_question_num.toString().split(',');
